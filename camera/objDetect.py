...
def getCenter(t0, t1, c):
    ...
    tc = t0
    cv2.drawContours(tc, cnt, -1, markColor, 2)
    cv2.rectangle(tc,(x,y),(x+w,y+h), markColor,2)
    cv2.imwrite('pic{}.jpg'.format(c), tc)
    
    return (x+0.5*w, y+0.5*h)

def getCenter_write(t0, t1, c):
    grey0 = cv2.cvtColor(t0, cv2.COLOR_BGR2GRAY)
    grey1 = cv2.cvtColor(t1, cv2.COLOR_BGR2GRAY)
    # blur0 = cv2.GaussianBlur(grey0,(7,7),0)
    # blur1 = cv2.GaussianBlur(grey1,(5,5),0)
...
def getTrashCan(t0, c):
    ...
    #cv2.imwrite("green{}.png".format(c), t1)
    return (x+0.5*w, y)
